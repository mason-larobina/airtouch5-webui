...
impl ZoneView {
    ...
    /// Whether a flag renders as the low-battery image
    /// (static/icons/battery-low.svg, shown via an <img> in zone.html) rather
    /// than a text label or Unicode glyph.
    pub fn flag_is_battery(&self, f: &str) -> bool {
        matches!(f, "LowBattery")
    }

    /// Glyph to render for a flag in place of its text label, when one is
    /// appropriate. Spill uses the droplet symbol (U+1F4A7); LowBattery is
    /// handled by `flag_is_battery` (an <img> to battery-low.svg) and
    /// returns None here. Other flags keep their text label (returned as
    /// None). Only HTML files use Unicode glyphs, so the symbols live in
    /// the template, not the Rust source.
